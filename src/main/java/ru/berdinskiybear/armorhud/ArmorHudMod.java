...
public final class ArmorHudMod {
    public static final String MOD_ID = "fixpots-armor-hud";
    ...
    public static Optional<Rect2i> getWidgetRect(GuiGraphicsExtractor graphics, Player player) {
        ...
        if (armorItems.isEmpty()) return Optional.empty();
        ...
        final int addedHotbarOffset = switch (config.getOffhandSlotBehavior()) {
            ...
            case ALWAYS_LEAVE_SPACE -> player.getMainArm() == config.getSide().asArm() ? ATTACK_INDICATOR_OFFSET : OFFHAND_OFFSET;
            case ADHERE -> {
                if (player.getMainArm() == config.getSide().asArm()) {
                    if (Minecraft.getInstance().options.attackIndicator().get() == AttackIndicatorStatus.HOTBAR && player.getAttackStrengthScale(0) < 1) {
        ...
        final int armorWidgetX = config.getOffsetX() * sideMultiplier + switch (config.getAnchor()) {
            ...
            case HOTBAR -> graphics.guiWidth() / 2 + ((HOTBAR_OFFSET + addedHotbarOffset) * sideMultiplier) + (widgetWidth * sideOffsetMultiplier);
        };

        final int armorWidgetY = switch (config.getAnchor()) {
            case BOTTOM, HOTBAR -> graphics.guiHeight() - widgetHeight - config.getOffsetY() - modCompat.screenSafeArea();
    ...
    public static Optional<Rect2i> getEffectiveWidgetRect(GuiGraphicsExtractor graphics, Player player) {
        ...
        return getWidgetRect(graphics, player).map(rect -> {
            if (config.getOrientation() == ArmorHudConfig.Orientation.HORIZONTAL) {
                ...
                if (config.isWarningShown()) additionalHeight += WARNING_SIZE + 2 + (config.getWarningBobIntensity() / 2);
                if (config.getDurabilityDisplay() != ArmorHudConfig.DurabilityDisplay.BAR) additionalHeight += Minecraft.getInstance().font.lineHeight;
                rect.setHeight(rect.getHeight() + additionalHeight);
                if (!config.getAnchor().isTop()) rect.setY(rect.getY() - additionalHeight);
            }
            return rect;
        });
    }
    ...
    public static boolean shouldPlayBreakSound(Player player) {
        for (int i = 0; i < SLOT_IDS.length; i++) {
            ...
            if (last.getDamageValue() != current.getDamageValue() && shouldShowWarning(current)) return true;
        }
        return false;
    }
